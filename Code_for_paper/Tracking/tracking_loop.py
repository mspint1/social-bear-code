...
#Pairwise tracking function
def run_pairwise_tracking(file1, file2, year,
                          min_delay=1441,
                          max_delay=10080,
                          max_el_time_min=90):
    # Load data
    ...
    TRACK_DIR_MAX = 45      # degrees
    GAP_MAX_MIN   = 120     # max minutes between consecutive interactions

    # Direction difference (0..180)
    df['abs_diff_direction'] = np.abs((df['p1_direction'] - df['p2_direction'] + 180) % 360 - 180)

    # Sort by time and compute gap to previous event
    df = df.sort_values("p1_t_start").reset_index(drop=True)
    df["gap_minutes"] = df["p1_t_start"].diff().dt.total_seconds() / 60

    # Aligned if direction ≤ threshold
    df["aligned"] = df["abs_diff_direction"] <= TRACK_DIR_MAX

    # Continues a run if aligned AND gap ≤ threshold
    df["continues_run"] = df["aligned"] & (df["gap_minutes"].fillna(np.inf) <= GAP_MAX_MIN)

    # Start a new run whenever we do NOT continue a run
    df["run_id"] = (~df["continues_run"]).cumsum()

    # Runs must have ≥ 2 aligned events to count as tracking
    run_sizes = df[df["aligned"]].groupby("run_id").size()
    valid_runs = set(run_sizes[run_sizes >= 2].index)

    # Keep ONLY events inside qualifying runs
    df["dyadic_tracking"] = df["aligned"] & df["run_id"].isin(valid_runs)

    # Keep only tracking rows
    filtered_df = df[df["dyadic_tracking"]].copy()
    ...
    # Summary metrics
    tracking_sequences = len(valid_runs)
    tracking_ellipses  = len(filtered_df)
    ...
    return {
        ...
        "Dyadic_Tracking_Events": tracking_ellipses,
    }
...
# Pairwise loop by year
for year in sorted(files_by_year.keys()):
    file_list = files_by_year[year]
    print(f"\n🔵 Processing {year} ({len(file_list)} bears)")
    for file1, file2 in combinations(file_list, 2):
        try:
            result = run_pairwise_tracking(file1, file2, year)
            if result:
                summary_records.append(result)
                print(f"🟡 {result['Bear1']} vs {result['Bear2']} — "
                      f"{result['Tracking_Sequences']} sequences / "
                      f"{result['Dyadic_Tracking_Events']} ellipses")
        except Exception as e:
            print(f"🔴 ERROR for {file1} vs {file2}: {e}")

# Save summary CSV
summary_df = pd.DataFrame(summary_records)
summary_df.to_csv("ORTEGA_Tracking_Summary.csv", index=False)
print("\n✅ Summary saved to ORTEGA_Tracking_Summary.csv")
...
# ───────────────────────────────────────────────
# Function to run tracking on a pair of files (NO PLOTTING)
def run_pairwise_tracking(file1, file2, year,
                          min_delay=0,
                          max_delay=10080,
                          max_el_time_min=90):
    ...
    end_time   = str(combined_df['study-timestamp'].max())
    if pd.isna(start_time) or pd.isna(end_time):
        print(f"⚠️ Skipping due to invalid time: {file1} vs {file2}")
        return None
    ...
    # ── Continuous tracking parameters
    TRACK_DIR_MAX = 45      # degrees
    GAP_MAX_MIN   = 120     # max minutes between consecutive ellipses in a run

    # Direction difference (0..180)
    df['abs_diff_direction'] = np.abs((df['p1_direction'] - df['p2_direction'] + 180) % 360 - 180)

    # Sort by time and compute gap to previous event (minutes)
    df = df.sort_values("p1_t_start").reset_index(drop=True)
    df["gap_minutes"] = df["p1_t_start"].diff().dt.total_seconds() / 60
    ...
    # Continues a run if aligned AND gap ≤ threshold (first row can't continue)
    df["continues_run"] = df["aligned"] & (df["gap_minutes"].fillna(np.inf) <= GAP_MAX_MIN)

    # Start a new run whenever we do NOT continue a run
    df["run_id"] = (~df["continues_run"]).cumsum()

    # Runs must have ≥ 2 aligned events to count as tracking (consecutive criterion)
    run_sizes = df[df["aligned"]].groupby("run_id").size()
    valid_runs = set(run_sizes[run_sizes >= 2].index)

    # Final tracking flag: ONLY events inside qualifying runs
    df["dyadic_tracking"] = df["aligned"] & df["run_id"].isin(valid_runs)

    # Keep only tracking rows (ellipses)
    filtered_df = df[df["dyadic_tracking"]].copy()
    ...
    # Summary metrics
    tracking_sequences = len(valid_runs)      # number of continuous runs
    tracking_ellipses  = len(filtered_df)     # number of ellipses inside qualifying runs
...
# Group files by year (e.g., 2020_ZZ_Positions.csv → '2020')
files_by_year = {}
for file in all_files:
    base = os.path.basename(file)
    parts = base.split("_")
...
# Save summary CSV
summary_df = pd.DataFrame(summary_records)
summary_df.to_csv("ORTEGA_cONSECUTIVE_Summary.csv", index=False)
print("\n✅ Summary saved to ORTEGA_Conesecutive_Summary.csv")
